...
// Optimized featured products data with better image paths
...
const FeaturedProducts = memo(() => {
  return (
    <section className="section bg-cream/50 px-4 sm:px-6 lg:px-8">
      <div className="container-custom">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 sm:mb-12">
          ...
          >
            ...
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-serif font-semibold text-softBlack leading-normal max-w-lg">Premium Ice Cream</h2>
          </motion.div>
        ...
        <div className="flex overflow-x-auto pb-4 gap-4 sm:gap-6 lg:gap-8 -mx-4 sm:mx-0 px-4 sm:px-0">
          {featuredProducts.map((product, index) => (
            <div className="flex-shrink-0 w-[240px] sm:w-[280px]" key={product.id}>
              <ProductCard
                product={product}
                index={index}
                priority={index < 3} // Priority loading for first 3 featured products
              />
            </div>
          ))}
        </div>
